...
function checkIfUserExists($username, $password){
	...
	$databaseQuery = "SELECT * FROM users WHERE username='$username' and password='$password'";
...
function checkIfUserAlreadyExists($username, $password){
	...
	//Create database query
	$databaseQuery = "SELECT * FROM users WHERE username='$username'";
...
function checkIfEmailExists($emailAddress){
	...
	//Create database query
	$databaseQuery = "SELECT * FROM users WHERE emailAddress='$emailAddress'";
...
function checkIfApproved($username, $password){
	...
	//Create database query
	$databaseQuery = "SELECT * FROM users WHERE username='$username' and password='$password' and confirmed='true'";
...
function insertUser($username, $password, $emailAddress){
	...
	//Check if email address is already registered
		...
		//Create query
		$databaseQuery = "INSERT INTO users $structure_login VALUES ('$username', '$password', '$emailAddress', '$hash', 'false')";
...
function authorizeUser($hash, $emailAddress){
	...
	$databaseQuery = "UPDATE users SET confirmed='true' WHERE hash='$hash'";
...
function getUnauthorizedUsers(){
	...
	//Create query
	$databaseQuery = "SELECT * FROM users WHERE confirmed='false'";
...
function deleteUser($hash){
	...
	//Create database query
	$databaseQuery = "DELETE FROM users WHERE hash='$hash'";
...
function resetPassword($ticket, $emailAddress, $newPassword){
	...
	while($row = mysqli_fetch_array($result)){
		...
		if ($checkTicket == $ticket){
			...
			$databaseQuery = "UPDATE users SET password='$newPassword' WHERE emailAddress='$emailAddress'";
			executeDatabase($databaseQuery);
			print ('reset');
		}
...
function changePassword($ticket, $username, $newPassword){
	...
	while($row = mysqli_fetch_array($result)){
		...
		if ($checkTicket == $ticket){
			$newPassword = saltPassword($newPassword);
			$databaseQuery = "UPDATE users SET password='$newPassword' WHERE emailAddress='$username'";
			executeDatabase($databaseQuery);
			print ('reset');
		}
